...
simu_full_data <- function(n){
  ...
  D1 <- rbinom(n, size = 1, prob = probD1)
  #simulate D(0)
  probD0 <- expit(0.4*X1+0.3*X2+0.4*X3+0.5*X4)
  D0 <- rbinom(n, size = 1, prob = probD0)
...
pqrc_point <- function(Z,D,Y,proscore,prinscore,om){
  ...
  pqr1 <- mean(prinscore0*Z*D*(Y-om11)/proscore+om11*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))/mean(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1)
  pqr0 <- mean(prinscore1*(1-Z)*D*(Y-om01)/(1-proscore)+om01*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))/mean(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1)
  pqrc <- pqr1-pqr0
  denom_est <- mean(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1)
  num_est <- mean(prinscore0*Z*D*(Y-om11)/proscore+om11*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))-
    mean(prinscore1*(1-Z)*D*(Y-om01)/(1-proscore)+om01*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))
...
ml_qr_var <- function(Z,D,Y,proscore,prinscore,om){
  ...
  denom_est <- mean(psi_D1*prinscore0+(psi_D0-prinscore0)*prinscore1)
  
  #Compute variance estimates
  pqr1 <- mean(prinscore0*Z*D*(Y-om11)/proscore+om11*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))/mean(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1)
  pqr0 <- mean(prinscore1*(1-Z)*D*(Y-om01)/(1-proscore)+om01*(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1))/mean(prinscore0*psi_D1+(psi_D0-prinscore0)*prinscore1)
  var_single <- mean((((psi_D0-prinscore0)*prinscore1*(om11-pqr1)+prinscore0*(psi_YD1-pqr1*psi_D1))-((psi_D1-prinscore1)*prinscore0*(om01-pqr0)+prinscore1*(psi_YD0-pqr0*psi_D0)))^2)/denom_est^2
...
eq_psw <- function(data, models){
  ...
  function(theta){
        ...
        c(pi_scores(theta[pi_pos]),
          ...
          Z*D*p0/proscore*(Y-theta[max(p1_pos)+1]),
          (1-Z)*D*p1/(1-proscore)*(Y-theta[max(p1_pos)+2]),
...
eq_qr <- function(data, models){
  ...
  function(theta){
    ...
    c(pi_scores(theta[pi_pos]),
      ...
      (psi_D0-p0)*p1*(om11-theta[max(m11_pos)+1])+p0*(psi_YD1-theta[max(m11_pos)+1]*psi_D1),
      (psi_D1-p1)*p0*(om01-theta[max(m11_pos)+2])+p1*(psi_YD0-theta[max(m11_pos)+2]*psi_D0),
...
cal_estimates <- function(df, truepce){
  ...
  ##################################################Weighting###############################################################
  ####Point estimate
  est_weighting1 <- mean(Z*D*prinscore0/proscore*Y)/mean(Z*D*prinscore0/proscore)
  est_weighting0 <- mean((1-Z)*D*prinscore1/(1-proscore)*Y)/mean((1-Z)*D*prinscore1/(1-proscore))
  ...
  pqr5 <- pqrc_point(Z,D,Y,proscore,list(prinscore1,mis_prinscore0),list(om11,mis_om01,mis_om10,mis_om00))
  pqr15 <- pqr5[1]
  pqr05 <- pqr5[2]
  pqrc5 <- pqr5[3]
  ...
  estimate_qr <- function(data, pi_formula, p0_formula, p1_formula, m01_formula, m11_formula, roots) {
  ...
  formula_list <- list(
    ...
    list(pi = Z ~ mis_X1+mis_X2+mis_X3+mis_X4, p0 = D ~ X1 + X2 + X3 + X4, p1 = D ~ X1 + X2 + X3 + X4, 
         m01 = Y ~ X1 + X2 + X3 + X4, m11 = Y ~ X1 + X2 + X3 + X4),
    list(pi = Z ~ X1 + X2 + X3 + X4, p0 = D ~ X1 + X2 + X3 + X4, p1 = D ~ mis_X1+mis_X2+mis_X3+mis_X4, 
         m01 = Y ~ X1 + X2 + X3 + X4, m11 = Y ~ mis_X1+mis_X2+mis_X3+mis_X4),
    list(pi = Z ~ X1 + X2 + X3 + X4, p0 = D ~ mis_X1+mis_X2+mis_X3+mis_X4, p1 = D ~ X1 + X2 + X3 + X4, 
         m01 = Y ~ mis_X1+mis_X2+mis_X3+mis_X4, m11 = Y ~ X1 + X2 + X3 + X4),
  ...
  #              formula_list[[i]]$m01, formula_list[[i]]$m11, theta_list[[i]])
  #})
  ...
  ptrc3 <- ptr3[3]
  #Misspecification scenario 3
  ptr4 <- ptrc_point(Z,D,Y,proscore,list(mis_prinscore1,prinscore0),list(mis_om11,om01,mis_om10,mis_om00))
  ptr14 <- ptr4[1]
  ptr04 <- ptr4[2]
  ptrc4 <- ptr4[3]
  #Misspecification scenario 4
  ptr5 <- ptrc_point(Z,D,Y,proscore,list(prinscore1,mis_prinscore0),list(om11,mis_om01,mis_om10,mis_om00))
  ptr15 <- ptr5[1]
  ptr05 <- ptr5[2]
  ptrc5 <- ptr5[3]
  #All models are wrongly specified
  ...
  theta_list_tr <- list(
    ...
    theta_tr4 = c(as.vector(coef(propen_m)), as.vector(coef(ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(om_m01)), as.vector(coef(mis_om_m11)), ptr14, ptr04, ptrc4),
    theta_tr5 = c(as.vector(coef(propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(ps_m1)), 
                  as.vector(coef(mis_om_m01)), as.vector(coef(om_m11)), ptr15, ptr05, ptrc5),
    theta_tr6 = c(as.vector(coef(mis_propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(mis_om_m01)), as.vector(coef(mis_om_m11)), ptr16, ptr06, ptrc6)
  )
  ...
  #})
  #  for (k in 1:6){
  #    print(round(lapply(estimates_tr, coef)[[k]]-theta_list_tr[[k]],5))
  #  }
  ...
  process_fold <- function(i, idc) {
    ...
    mlqck4 <- pqrc_point(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                         ...
                         list(ml_om_m11[[2]], ml_om_m01[[1]], ml_om_m10[[2]], ml_om_m00[[2]]))
    mlqck_num4 <- mlqck4[4]
    mlqck_denom4 <- mlqck4[5]

    mlqck5 <- pqrc_point(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                         list(ml_ps_m1[[1]], ml_ps_m0[[2]]), 
                         list(ml_om_m11[[1]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]))
    mlqck_num5 <- mlqck5[4]
...
write.csv(out_table, file= "sim_table_11_case1.csv")
