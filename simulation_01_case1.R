...
pqrc_point <- function(Z,D,Y,proscore,prinscore,om){
  ...
  pqr1 <- mean((1-prinscore0)*Z*D*(Y-om11)/proscore+om11*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))/mean(((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
  pqr0 <- mean(prinscore1*(1-Z)*(1-D)*(Y-om00)/(1-proscore)+om00*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))/mean(((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
  pqrc <- pqr1-pqr0
  denom_est <- mean(((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
  num_est <- mean((1-prinscore0)*Z*D*(Y-om11)/proscore+om11*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))-
    mean(prinscore1*(1-Z)*(1-D)*(Y-om00)/(1-proscore)+om00*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
...
ml_qr_var <- function(Z,D,Y,proscore,prinscore,om){
  ...
  pqr1 <- mean((1-prinscore0)*Z*D*(Y-om11)/proscore+om11*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))/mean(((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
  pqr0 <- mean(prinscore1*(1-Z)*(1-D)*(Y-om00)/(1-proscore)+om00*((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))/mean(((1-prinscore0)*psi_D1+(-psi_D0+prinscore0)*prinscore1))
  var_single <- mean(((prinscore1*(prinscore0-psi_D0)*(om11-pqr1)+(1-prinscore0)*(psi_YD1-pqr1*psi_D1))-((1-prinscore0)*(psi_D1-prinscore1)*(om00-pqr0)+prinscore1*(psi_Y1_D0-pqr0*(1-psi_D0))))^2)/denom_est^2
...
eq_psw <- function(data, models){
  ...
  function(theta){
    ...
    c(pi_scores(theta[pi_pos]),
      ...
      Z*D*(1-p0)/proscore*(Y-theta[max(p1_pos)+1]),
      (1-Z)*(1-D)*p1/(1-proscore)*(Y-theta[max(p1_pos)+2]),
...
eq_qr <- function(data, models){
  ...
  function(theta){
    ...
    c(pi_scores(theta[pi_pos]),
      ...
      (p0-psi_D0)*p1*(om11-theta[max(m11_pos)+1])+(1-p0)*(psi_YD1-theta[max(m11_pos)+1]*psi_D1),
      (psi_D1-p1)*(1-p0)*(om00-theta[max(m11_pos)+2])+p1*(psi_Y1_D0-theta[max(m11_pos)+2]*(1-psi_D0)),
...
cal_estimates <- function(df, truepce){
  ...
  ##################################################Weighting###############################################################
  ####Point estimate
  est_weighting1 <- mean(Z*D*(1-prinscore0)/proscore*Y)/mean(Z*D*(1-prinscore0)/proscore)
  est_weighting0 <- mean((1-Z)*(1-D)*prinscore1/(1-proscore)*Y)/mean((1-Z)*(1-D)*prinscore1/(1-proscore))
  ...
  estimate_qr <- function(data, pi_formula, p0_formula, p1_formula, m00_formula, m11_formula, roots) {
  ...
  formula_list <- list(
    ...
    list(pi = Z ~ X1 + X2 + X3 + X4, p0 = D ~ X1 + X2 + X3 + X4, p1 = D ~ mis_X1+mis_X2+mis_X3+mis_X4, 
         m00 = Y ~ X1 + X2 + X3 + X4, m11 = Y ~ mis_X1+mis_X2+mis_X3+mis_X4),
    list(pi = Z ~ X1 + X2 + X3 + X4, p0 = D ~ mis_X1+mis_X2+mis_X3+mis_X4, p1 = D ~ X1 + X2 + X3 + X4, 
         m00 = Y ~ mis_X1+mis_X2+mis_X3+mis_X4, m11 = Y ~ X1 + X2 + X3 + X4),
  ...
  theta_list_tr <- list(
    ...
    theta_tr4 = c(as.vector(coef(propen_m)), as.vector(coef(ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(om_m00)), as.vector(coef(mis_om_m11)), ptr14, ptr04, ptrc4),
    theta_tr5 = c(as.vector(coef(propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(ps_m1)), 
                  as.vector(coef(mis_om_m00)), as.vector(coef(om_m11)), ptr15, ptr05, ptrc5),
    theta_tr6 = c(as.vector(coef(mis_propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(mis_om_m00)), as.vector(coef(mis_om_m11)), ptr16, ptr06, ptrc6)
  )
...
write.csv(out_table, file= "sim_table_01_case1.csv")
