...
pqrc_point00 <- function(Z,D,Y,proscore,prinscore,om,theta){
  ...
  tau <- lambda + (theta-1)/sqrt(delta)*((psi_D0-prinscore0)*(2*(prinscore1+(theta-1)*(1-prinscore0)-sqrt(delta))-lambda)+(psi_D1-prinscore1)*(2*(prinscore0+(theta-1)*(1-prinscore1)-sqrt(delta))-lambda))
...
eq_psw00 <- function(data, models, oddsratio){
  Z <- data$Z
  D <- data$D
  Y <- data$Y
  ...
  if (oddsratio>9999){
    function(theta){
      ...
      c(pi_scores(theta[pi_pos]),
        ...
        (1-D)*Z*Y/proscore-theta[max(p1_pos)+1]*(1-p1),
        (1-p1)*((1-D)*(1-Z)*Y/(1-p0)/(1-proscore)-theta[max(p1_pos)+2]),
...
eq_qr00 <- function(data, models, oddsratio){
  Z <- data$Z
  D <- data$D
  Y <- data$Y
  ...
  function(theta){
    ...
    delta <- (1+(oddsratio-1)*(p1+p0))^2-4*oddsratio*(oddsratio-1)*p1*p0
    lambda <- 1+(oddsratio-1)*(2-p0-p1)-sqrt(delta)
    tau <- lambda + (oddsratio-1)/sqrt(delta)*((psi_D0-p0)*(2*(p1+(oddsratio-1)*(1-p0)-sqrt(delta))-lambda)+(psi_D1-p1)*(2*(p0+(oddsratio-1)*(1-p1)-sqrt(delta))-lambda))
...
cal_estimates00 <- function(df, theta, truepce){
  ...
  if (theta>9999){
    est_weighting1 <- mean(Z*(1-D)/proscore*Y)/mean(1-prinscore1)
    est_weighting0 <- mean((1-Z)*(1-D)*(1-prinscore1)/(1-proscore)/(1-prinscore0)*Y)/mean(1-prinscore1)
    ...
    lambda <- 1+(theta-1)*(2-prinscore0-prinscore1)-sqrt(delta)
    mis_lambda <- 1+(theta-1)*(2-mis_prinscore0-mis_prinscore1)-sqrt(mis_delta)
  ...
  estimate_psw <- function(data, theta){
    ...
    m_estimate(estFUN = eq_psw00, data = data,
  ...
  calculate_pqrc <- function(proscore, prinscore, om, theta) {
    pqrc_point00(Z, D, Y, proscore, prinscore, om, theta)
  }
  
  # Function to calculate and return the results for all working design matrices
  calculate_all_cases <- function(theta) {
    list(
      all_correct = pqrc_point00(Z, D, Y, proscore, prinscore, om, theta),
      pi_p = pqrc_point00(Z, D, Y, proscore, prinscore, mis_om, theta),
      p_om = pqrc_point00(Z, D, Y, mis_proscore, prinscore, om, theta),
      pi_om = pqrc_point00(Z, D, Y, proscore, mis_prinscore, om, theta),
      all_mis = pqrc_point00(Z, D, Y, mis_proscore, mis_prinscore, mis_om, theta)
    )
  }
  ...
  estimate_qr <- function(data, pi_formula, p0_formula, p1_formula, m00_formula, m10_formula, roots, oddsratio) {
  ...
  theta_list <- lapply(seq_along(theta_com), function(i) {
    list(
      theta_qr1 = c(as.vector(coef(propen_m)), as.vector(coef(ps_m0)), as.vector(coef(ps_m1)), 
                    as.vector(coef(om_m00)), as.vector(coef(om_m10)), pqr11[i], pqr01[i], pqrc1[i]),
      theta_qr2 = c(as.vector(coef(propen_m)), as.vector(coef(ps_m0)), as.vector(coef(ps_m1)), 
                    as.vector(coef(mis_om_m00)), as.vector(coef(mis_om_m10)), pqr12[i], pqr02[i], pqrc2[i]),
      theta_qr3 = c(as.vector(coef(mis_propen_m)), as.vector(coef(ps_m0)), as.vector(coef(ps_m1)), 
                    as.vector(coef(om_m00)), as.vector(coef(om_m10)), pqr13[i], pqr03[i], pqrc3[i]),
      theta_qr4 = c(as.vector(coef(propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                    as.vector(coef(om_m00)), as.vector(coef(om_m10)), pqr14[i], pqr04[i], pqrc4[i]),
      theta_qr5 = c(as.vector(coef(mis_propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                    as.vector(coef(mis_om_m00)), as.vector(coef(mis_om_m10)), pqr15[i], pqr05[i], pqrc5[i])
    )
  })
  ...
  se_list <- mclapply(c(1:5), function(i) {
    lapply(c(1:3), function(j) {
      ...
      estimate <- estimate_qr(df, formula_list[[i]]$pi, formula_list[[i]]$p0, formula_list[[i]]$p1, 
                              formula_list[[i]]$m00, formula_list[[i]]$m10, theta_list[[j]][[i]], oddsratio = theta_com[j])
  ...
  ptr1 <- ptrc_point00(Z,D,Y,proscore,prinscore,om)
  ptr11 <- ptr1[1]
  ptr01 <- ptr1[2]
  ptrc1 <- ptr1[3]
  # pi+p
  ptr2 <- ptrc_point00(Z,D,Y,proscore,prinscore,mis_om)
  ptr12 <- ptr2[1]
  ptr02 <- ptr2[2]
  ptrc2 <- ptr2[3]
  # p+om
  ptr3 <- ptrc_point00(Z,D,Y,mis_proscore,prinscore,om)
  ptr13 <- ptr3[1]
  ptr03 <- ptr3[2]
  ptrc3 <- ptr3[3]
  # pi+om
  ptr4 <- ptrc_point00(Z,D,Y,proscore,mis_proscore,om)
  ptr14 <- ptr4[1]
  ptr04 <- ptr4[2]
  ptrc4 <- ptr4[3]
  # All models are wrongly specified
  ptr5 <- ptrc_point00(Z,D,Y,mis_proscore,mis_prinscore,mis_om)
  ptr15 <- ptr5[1]
  ptr05 <- ptr5[2]
  ptrc5 <- ptr5[3]
  ...
  theta_list_tr <- list(
    ...
    theta_tr4 = c(as.vector(coef(propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(om_m00)), as.vector(coef(om_m10)), ptr14, ptr04, ptrc4),
    theta_tr5 = c(as.vector(coef(mis_propen_m)), as.vector(coef(mis_ps_m0)), as.vector(coef(mis_ps_m1)), 
                  as.vector(coef(mis_om_m00)), as.vector(coef(mis_om_m10)), ptr15, ptr05, ptrc5)
  )
  ...
  process_fold <- function(i, idc, theta_com) {
    ...
    for (j in seq_along(theta_com)) {
      ...
      mlqck1 <- pqrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
      ...
      mlqck2 <- pqrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                           list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
      ...
      mlqck_var1 <- ml_qr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                              list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                              list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]), theta_com[j])
      
      mlqck_var2 <- ml_qr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                              list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                              list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]), theta_com[j])
      
      mlqck_var3 <- ml_qr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                              list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                              list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]), theta_com[j]);
      
      mlqck_var4 <- ml_qr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                              list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                              list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]), theta_com[j]);
      
      mlqck_var5 <- ml_qr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                              list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                              list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]), theta_com[j]);
    ...
    # MLinfty
    mltck1 <- ptrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                         list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                         list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    mltck_num1 <- mltck1[4]
    mltck_denom1 <- mltck1[5]
    
    mltck2 <- ptrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                         list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                         list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]))
    mltck_num2 <- mltck2[4]
    mltck_denom2 <- mltck2[5]
    
    mltck3 <- ptrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                         list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                         list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    mltck_num3 <- mltck3[4]
    mltck_denom3 <- mltck3[5]
    
    mltck4 <- ptrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                         list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                         list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    mltck_num4 <- mltck4[4]
    mltck_denom4 <- mltck4[5]
    
    mltck5 <- ptrc_point00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                         list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                         list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]))
    mltck_num5 <- mltck5[4]
    ...
    mltck_var1 <- ml_tr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                            list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                            list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    
    mltck_var2 <- ml_tr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                            list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                            list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]))
    
    mltck_var3 <- ml_tr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                            list(ml_ps_m1[[1]], ml_ps_m0[[1]]), 
                            list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    
    mltck_var4 <- ml_tr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[1]], 
                            list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                            list(ml_om_m11[[1]], ml_om_m01[[1]], ml_om_m10[[1]], ml_om_m00[[1]]))
    
    mltck_var5 <- ml_tr_var00(Z[-idx_F], D[-idx_F], Y[-idx_F], ml_proscore[[2]], 
                            list(ml_ps_m1[[2]], ml_ps_m0[[2]]), 
                            list(ml_om_m11[[2]], ml_om_m01[[2]], ml_om_m10[[2]], ml_om_m00[[2]]))   
