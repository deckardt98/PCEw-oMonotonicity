...
cal_estimates <- function(df, truepce){
  ...
  estimate_psw <- function(data){
    ...
    m_estimate(estFUN = eq_psw, data = data,
               ...
               #root_control = setup_root_control(start = theta_ps+rnorm(length(theta_ps),mean=0,sd=0.1))
               compute_roots = FALSE,
               roots = theta_ps,
  ...
  #round(coef(estimate_psw(data=df))-theta_ps,6)
...
write.csv(out_table, file= "sim_table_01_case2.csv")
